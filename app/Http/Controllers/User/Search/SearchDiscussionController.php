...
class SearchDiscussionController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $toExclude = [\App\Models\Product::class, \App\Models\News::class, \App\Models\Gallery::class];
        ...
        $results = collect($files)->map(function (SplFileInfo $file) {
            ...
            return $model::search($keyword)->where('is_approved', 1)->take(3)->get()->map(function ($modelRecord) use ($model, $fields, $keyword, $classname) {
                ...
                $modelRecord->setAttribute('match', substr($serializedValues, 0, 20) . '...');
    ...
    private function resolveModelViewLink(Model $model)
    {
        ...
        $mapping = [
            \App\Models\Discussion::class => '/forum/{slug}'
        ];
