...
class SearchProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $toExclude = [\App\Models\Gallery::class, \App\Models\News::class, \App\Models\Discussion::class];
        ...
        $results = collect($files)->map(function (SplFileInfo $file) {
            ...
            return $model::search($keyword)->take(3)->get()->map(function ($modelRecord) use ($model, $fields, $keyword, $classname) {
                ...
                // use $slice as the match, otherwise if undefined we use the first 60 character of serialisedValues
                $modelRecord->setAttribute('match', substr($serializedValues, 0, 60) . '...');
    ...
    private function resolveModelViewLink(Model $model)
    {
        ...
        $mapping = [
            \App\Models\Product::class => '/produk/{slug}'
        ];
        ...
        if (Arr::has($mapping, $modelClass)) {
            return URL::to(str_replace('{slug}', $model->slug, $mapping[$modelClass]));
        }
        ...
        return URL::to('/' . $modelName . '/' . $model->slug);
    }
}
